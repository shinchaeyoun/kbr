...
const UserDetail = () => {
  ...
  const getUserInfo = async () => {
    const resp = await (
      await axios.get(`http://192.168.23.2:5001/user/list/`)
    ).data[idx];
    setUser(resp);
  };
  ...
  const handleSubmit = async () => {
    await axios
      .patch(`http://192.168.23.2:5001/user/update?idx=${index}`, user)
      .then((res) => {
        console.log("=========res", res);
  ...
  const userDelete = async () => {
    ...
    if (deletConfirm) {
      await axios
        .delete(`http://192.168.23.2:5001/user/delete?idx=${index}`, user)
        .then((res) => {
          alert("삭제되었습니다.");
