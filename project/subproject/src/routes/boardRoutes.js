...
router.get("/", (req, res) => {
  ...
  const sqlDESC = `SELECT * FROM project ORDER BY idx DESC LIMIT ?, ?`; // 최신순
  const sqlASC = `SELECT * FROM project ORDER BY idx ASC LIMIT ?, ?`; // 오래된순
  const sql = `SELECT * FROM project WHERE idx = ?`;
...
router.post("/upload", async (req, res) => {
  ...
  if (index === undefined) {
    const sqlDESC = `SELECT * FROM project ORDER BY idx DESC LIMIT 0, 1`;
  ...
  fs.writeFile(filePathSubproject, base64Data, { encoding: "base64" }, (err) => {
    ...
    fs.writeFile(filePathKbrainCdms, base64Data, { encoding: "base64" }, (err) => {
      ...
      const sql = "UPDATE project SET thumb = ? WHERE idx = ?";
