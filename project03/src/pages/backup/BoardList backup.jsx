...
const BoardList = (props) => {
  ...
  const [isModalIdx, setIsModalIdx] = useState();
  ...
  const onSearch = async () => {
    await axios.post(`http://192.168.23.65:5000/board/search`, isSearch).then((res) => setboardList(res.data));
  };
  ...
  const moveToUpdate = (idx, e) => {
    if (level > 2) {
      navigate("/board/update/" + idx, {state : {index: idx}});
    } else {
      alert("수정 권한 없음");
    }
  };
  ...
  const openPup = (link,title) => {
    const popup = window.open(`${link}`, `${title}`, 'width=1300px,height=800px,scrollbars=yes');
  };
  ...
  return (
    <div className="board-list">
      <Modal isModalOpen={isModalOpen} board={boardList} idx={isModalIdx} setIsModalOpen={setIsModalOpen}/>
      ...
      {boardList.map((board) => (
        <S.BoardItem key={board.idx}>
          ...
          <div className="outerUrl" onClick={()=>{openPup(board.outerUrl,board.title)}}>
            <span>URL</span>
            {board.outerUrl}
          </div>
