...
const User = (props) => {
  ...
  const getUserInfo = async () => {
    const resp = await axios.get(`http://192.168.23.2:5100/user?idx=${idx}`);
    setUser(resp.data);
  };
  ...
  const handleSubmit = async () => {
    ...
    await axios
      .patch(`http://192.168.23.2:5100/user/update?idx=${idx}`, user)
      .then((res) => {
        alert("수정되었습니다.");
  ...
  const userDelete = async () => {
    ...
    if (deletConfirm) {
      await axios
        .delete(`http://192.168.23.2:5100/user/delete?idx=${idx}`, user)
        .then((res) => {
          alert("삭제되었습니다.");
          setIsModalOpen(false);
        });
