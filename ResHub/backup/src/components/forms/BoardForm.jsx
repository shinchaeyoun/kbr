...
const BoardForm = ({
  ...
  const { year, title, customer, innerUrl, outerUrl, description, thumb } =
    board;
  ...
  const getBoard = async () => {
    if (mode == "view") {
      await axios
        .get(`http://192.168.23.2:5100/project?code=${idx}`)
        .then((res) => {
      ...
      reader.onload = async () => {
        ...
        try {
          const response = await axios.post(
            `http://192.168.23.2:5100/project/upload`,
            {
              base64Image,
              originalName,
              idx, // 게시글의 idx
  ...
  const handleSubmit = async () => {
    ...
    // 중복 체크
    const response = await axios.post(`http://192.168.23.2:5100/project/dup`, {
      year: year,
      title: title,
    });
    if (response.data.idx !== idx && response.data.result) {
      ...
      // 중복된 데이터가 존재할 경우
      console.log("response.data.idx ==>", response.data.idx ,idx);
    ...
    try {
      if (mode === "view") {
        const response = await axios.patch(
          `http://192.168.23.2:5100/project/update?idx=${idx}`,
          updatedBoard,
          {
            headers: {
        ...
        const response = await axios.post(
          `http://192.168.23.2:5100/project`,
          updatedBoard,
          {
            headers: {
              "Content-Type": "application/json",
  ...
  const deleteBoard = async () => {
    ...
    if (userInput === deleteCode) {
      try {
        await axios.delete(`http://192.168.23.2:5100/project/delete`, {
          data: { idx },
        });
        setIsModalOpen(false);
