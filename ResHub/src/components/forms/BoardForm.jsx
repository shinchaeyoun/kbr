...
const BoardForm = ({
  ...
  const getBoard = async () => {
    if (mode == "view") {
      await axios
        .get(`http://192.168.23.2:5000/project?code=${idx}`)
        .then((res) => {
      ...
      reader.onload = async () => {
        ...
        try {
          const response = await axios.post(
            `http://192.168.23.2:5000/project/upload`,
            {
              base64Image,
              originalName,
              idx, // 게시글의 idx
  ...
  const handleSubmit = async () => {
    ...
    // 중복 체크
    const response = await axios.post(`http://192.168.23.2:5000/project/dup`, {
      year: year,
      title: title,
    });
    ...
    try {
      if (mode === "view") {
        const response = await axios.patch(
          `http://192.168.23.2:5000/project/update?idx=${idx}`,
          updatedBoard,
          {
            headers: {
        ...
        const response = await axios.post(
          `http://192.168.23.2:5000/project`,
          updatedBoard,
          {
            headers: {
              "Content-Type": "application/json",
