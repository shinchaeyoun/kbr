...
router.get("/", async (req, res) => {
  ...
  if (!subjectId || subjectId === null) {
    sqlDESC = `SELECT * FROM board WHERE projectCode = ? AND (subjectId IS NULL OR subjectId = 'NULL') ORDER BY idx DESC LIMIT ?, ?`; // 최신순
    params = [projectCode, offset, limit];
  } else {
    sqlDESC = `SELECT * FROM board WHERE projectCode = ? AND subjectId = ? AND category = ? ORDER BY idx DESC LIMIT ?, ?`;
...
router.get("/detail", async (req, res) => {
  const detailIndex = req.query.boardIndex ? Number(req.query.boardIndex) : null; // 게시판 인덱스
...
router.post("/", upload.array("attachment"), (req, res) => {
  ...
  if (subjectId == "board") {
    ...
    console.log("과목 게시판에 글 추가: else");
  }

  query(sql, params)
    .then((data) => {
      log("게시글 추가 성공:", data);
      res.send({ msg: "전송 성공", data: data });
    })
    .catch((err) => {
      log("게시글 추가 실패:", err);
...
router.patch("/update", upload.array("attachment"), async (req, res) => {
  ...
  console.log("게시판 글 수정 요청:", req.body);
  ...
  `;
  query(sql, params)
    .then((data) => res.send({ msg: "수정 성공", data }))
