...
const Signup = ({ setShowSignup }) => {
  ...
  const checkUserId = async () => {
    ...
    await axios
      .get(`http://192.168.23.2:5001/auth/signup?id=${data.id}`)
      .then((res) => {
  ...
  const onChange = (e) => {
    ...
    if (name === "pwCheck") {
      setPwCheck(value);
      setNotMatch(value !== data.password);
    } else {
      const nextData = {
        ...data,
        [name]: value,
      ...
      name === "password" && setNotMatch(value !== pwCheck);
    }
  };
  ...
  const onSubmit = async () => {
    ...
    await axios.post("http://192.168.23.2:5001/auth/signup", data).then(() => {
