...
router.post("/search", (req, res) => {
  ...
  const sql1 = `
          SELECT * FROM board
          WHERE title LIKE ? OR subTitle LIKE ? OR customer LIKE ? OR pm1 LIKE ? OR pm2 LIKE ? OR pm3 LIKE ?
          ORDER BY idx DESC
          LIMIT 0, 10;
        `;
  ...
  if (searchValue !== undefined) {
    ...
    query(sql1, [searchPattern,searchPattern,searchPattern,searchPattern,searchPattern,searchPattern])
      .then((data) => {
        res.send(data)
      })
