...
router.post("/upload", upload.single("image"), (req, res) => {
  ...
  if (!file) {
    return res.status(400).send({ msg: "파일이 업로드되지 않았습니다." });
  }

  const sql = "UPDATE board SET thumb = ? WHERE idx = ?";
  const thumb = `images/thumbnail/${file.filename}`;
  const index = req.query.idx;

  console.log(req.query.idx);

  query(sql, [thumb, index])
    .then((data) => {
      console.log("저장된 데이터", data);
      res.send({ thumb });
    })
    .catch((err) => {
      console.log("upload 전송 실패===catch===", err);
      res.send({ msg: "전송 실패" });
    });

  // 파일 정보 처리
  console.log(`파일 이름: ${file.originalname}`);
  console.log(`저장된 경로: ${file.path}`);
});
