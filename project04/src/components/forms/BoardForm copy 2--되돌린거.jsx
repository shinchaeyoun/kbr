...
const BoardForm = ({ mode, idx, isModalOpen, setIsModalOpen }) => {
  ...
  const handleFileUpload = async (file) => {
    ...
    reader.onload = async () => {
      ...
      if (filePath !== null) {
        console.log("if", filePath !== null, filePath);
        console.log("원본파일명", filePath.split("/")[4].split(".")[0]);

        // fileName = filePath.split("/")[3].split(".")[0];
        // setFileName(filePath.split("/")[3].split(".")[0]);
        // setFileName(filePath.split("/")[3].split(".")[0]);
        fileName = filePath.split("/")[4].split(".")[0];
      } else {
        // filePath가 null 일 때. 이미지 값이 애초에 없었을 때때
        console.log("else", file.name);
      ...
      console.log(
        "새로 올리는 파일명 = originalName //",
        originalName.split(".")[0]
      );
      console.log("새로 올리는 파일명 = fileName //", fileName);
